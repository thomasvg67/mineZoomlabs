...
exports.sendEmail = async (req, res) => {
  try {
    ...
    const from = req.user?.email || req.user?.uname;
if (!from) {
...
const ccStr = typeof cc === 'string' ? cc : '';

const toList = toStr.split(',').map(e => e.trim()).filter(Boolean);
const ccList = ccStr.split(',').map(e => e.trim()).filter(Boolean);
...
exports.markAsRead = async (req, res) => {
  try {
    ...
    const userEmail = req.user?.email || req.user?.uname;
